        ...
        <div class="tb_box">
          ...
          >
            ...
            <div v-if="item.type === 'input'">
              <input
                :placeholder="item.ph || '请填写'"
                @input="change($event, ind, 'input', item.code)"
                v-model="ite[item.code]"
                type="text"
              />
            </div>
            ...
            <div v-if="item.type === 'checkbox'">
              <checkbox-group
                @change="change($event, ind, 'checkout', item.code)"
                v-model="ite[item.code]"
              >
                <checkbox
                  :value="item.code"
                  :checked="ite[item.code] === 1"
            ...
            <div v-if="item.type === 'select'">
              <picker
                placeholder="请选择"
                @change="change($event, ind, 'select', item.code, item.list)"
                range-key="label"
                :value="ite[item.code]"
                :range="item.list"
              >
            ...
            <div v-if="item.type === 'edit'">
              <template v-if="tableDataList.length > 1">
                <i
                  ...
                  style="font-size: 44rpx; color: #676fdf"
                  @click="change($event, ind, 'edit', item.code)"
                ></i>
              </template>
            </div>
          </div>
...
<style lang="scss" scoped>
  ...
  .diyTable {
    .table_box {
      ...
      > .tb_box:first-child {
        background: rgba(103, 111, 223, 0.05);
        .th {
